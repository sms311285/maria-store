...
class CartProduct extends ChangeNotifier {
  ...
  // Construtor para criar/pegar os dados do firebase dos dados do carrinho, transformando os itens do carrinho em objetos CartProduct
  ...
  // Metodo para slavar os dados (itens do carrinho no user), transaformando em map pois é a forma adotada no projeto para salvar (Chama lá no CartManager)
  ...
  // criado o mapa de itens para pedido, separado do toCartItemMap por conta do preço fixo
  ...
  void increment() {
    quantity = quantity! + 1;
    // Notificando as mudanças lá no CartTile
    notifyListeners();
  }
  ...
  // Verificando se tem estoque disponível para impedir incluir intens sem estoque no carrinho
  bool get hasStock {
    // verificando se prd não é null e se o prd foi deletado já retorna a msg de estoque insuficiente
    if (product != null && product!.deleted!) return false;
    // Chama o get itemSize para pegar o tamanho do item correspondente
    final size = itemSize;
    // Se não encontrou o item que queremos retorna false, não tem estoque
    if (size == null) return false;
    // Retorna a qtde que queremos comprar
